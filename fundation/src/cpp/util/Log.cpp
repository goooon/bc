...
static char* Log_formatTraceEntry(traceEntry* cur_entry)
{
	...
	{
		char* format = "empty messaage";//Messages_get(cur_entry->number, cur_entry->level);
...
static void Log_output(int log_level, char* msg)
{
	...
	if (trace_callback)
		(*trace_callback)((LOG_LEVELS)log_level, msg);
}
